...
App.controller('CorrespondenceOUTController', ["Utility", "api", "ComboBoxHandler", "$scope", "$rootScope", "$window", "Upload", "$sce",
function (Utility, api, ComboBoxHandler, $scope, $rootScope, $window, Upload, $sce) {
    ...
    $scope.Open = function (id) {
        ...
        $window.location.href = "/correspondenceOUT/open/";
    }
    $scope.Update = function (id) {
        sessionStorage.correspondence_id = id;
        $window.location.href = "/correspondenceOUT/update/";
    }
    $scope.Create = function () {
        $window.location.href = "/correspondenceOUT/create/";
    }
    ...
    $scope.OpenCorrespondence = function (id) {
        ...
        api.Get("/correspondenceOUT/openCorrespondence/" + id).success(function (Response) {
            $scope.model.correspondence = Response[0];

            //convert dates to readable format
            $scope.model.correspondence.FROM_OFFICE_SENT_DATE = new Date(Utility.FixJsonDate($scope.model.correspondence.FROM_OFFICE_SENT_DATE));
            $scope.model.correspondence.RECIEPT_DATE = new Date(Utility.FixJsonDate($scope.model.correspondence.RECIEPT_DATE));
            $scope.model.correspondence.REPLY_DATE = new Date(Utility.FixJsonDate($scope.model.correspondence.REPLY_DATE));
            $scope.model.correspondence.ARCHIVE_OUT_NUMBER_DATE = new Date(Utility.FixJsonDate($scope.model.correspondence.ARCHIVE_OUT_NUMBER_DATE));
    ...
    $scope.CreateCorrespondence = function () {
        ...
        $("#loading").css("display", "block");
        api.Post("/correspondenceOUT/createCorrespondence/", $scope.model.correspondence).success(function (Response) {
    ...
    $scope.UpdateCorrespondence = function () {
        $scope.model.correspondence.CREATED_DATE = new Date();
        $("#loading").css("display", "block");
        api.Post("/correspondenceOUT/updateCorrespondence/", $scope.model.correspondence).success(function (Response) {
            if (Response != null && Response != 0)
                $scope.uploadFiles(Response); //response is returned with correspondence id
        }).error(function (error) { console.log(error); })
    }
    ...
    $scope.DeleteCorrespondence = function (id) {
        $("#loading").css("display", "block");
        api.Post("/correspondenceOUT/deleteCorrespondence/" + id).success(function (Response) {
            if (Response == "True") {
    ...
    $scope.SearchCorrespondence = function () {
        $("#loading").css("display", "block");
        api.Get("/correspondenceOUT/searchCorrespondence/", { keyword: $scope.model.searchKeyword, correspondenceType: $scope.model.correspondence.IN_OUT, pageNumber: $scope.model.tableCurrentPage }).success(function (Response) {
    ...
    $scope.uploadFiles = function (id) {
        ...
        if (files && files.length) {
            Upload.upload({
                url: '/correspondenceOUT/uploadFile/' + id,
                data: {
                    files: files
                }
